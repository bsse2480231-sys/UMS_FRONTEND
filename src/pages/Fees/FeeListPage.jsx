...
//   const { register, handleSubmit, formState: { errors, isSubmitting }, reset } = useForm({ resolver: yupResolver(feeSchema) });

//   const fetchFees = async () => {
//     try { setLoading(true); const res = await getFees(); setFees(res.data); } 
//     catch { toast.error('Failed to fetch fees'); } finally { setLoading(false); }
//   };

//   useEffect(() => { fetchFees(); }, []);

//   const openCreateModal = () => { setEditingFee(null); reset({ fee_id: '', student_id: '', amount: '', fee_type: '', due_date: '', status: 'Unpaid' }); setIsModalOpen(true); };
...
//                 <td className="py-4 px-6 text-sm text-slate-600">{f.student_first} {f.student_last}</td>
...
//           <div><label className="block text-sm font-medium text-slate-600 mb-1">Fee ID</label><input {...register('fee_id')} disabled={editingFee} className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 disabled:bg-slate-100" placeholder="FEE002" />{errors.fee_id && <p className="text-red-500 text-xs mt-1">{errors.fee_id.message}</p>}</div>
//           <div><label className="block text-sm font-medium text-slate-600 mb-1">Student ID</label><input {...register('student_id')} disabled={editingFee} className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 disabled:bg-slate-100" placeholder="STU001" />{errors.student_id && <p className="text-red-500 text-xs mt-1">{errors.student_id.message}</p>}</div>
...
//               <select {...register('fee_type')} className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white">
...
const FeeListPage = () => {
  ...
  const [studentOptions, setStudentOptions] = useState([]); // Formatted for SearchableSelect
  const [loading, setLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingFee, setEditingFee] = useState(null);
  ...
  const fetchDropdownData = async () => {
    try {
      ...
      // Format for SearchableSelect: { value: 'STU001', label: 'STU001 - Alice Smith' }
