...
//     try { setLoading(true); const res = await getExams(); setExams(res.data); } 
//     catch { toast.error('Failed to fetch'); } finally { setLoading(false); }
//   };

//   useEffect(() => { fetchExams(); }, []);

//   const openCreateModal = () => { setEditingExam(null); reset({ exam_id: '', course_id: '', exam_type: '', exam_date: '', total_marks: '', classroom_id: '' }); setIsModalOpen(true); };
...
//           <div><label className="block text-sm font-medium text-slate-600 mb-1">Exam ID</label><input {...register('exam_id')} disabled={editingExam} className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 disabled:bg-slate-100" placeholder="EXM002" />{errors.exam_id && <p className="text-red-500 text-xs mt-1">{errors.exam_id.message}</p>}</div>
...
//             <div><label className="block text-sm font-medium text-slate-600 mb-1">Classroom ID</label><input {...register('classroom_id')} className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300" placeholder="ROOM001" />{errors.classroom_id && <p className="text-red-500 text-xs mt-1">{errors.classroom_id.message}</p>}</div>
//           </div>
//           <div className="grid grid-cols-2 gap-4">
//             <div><label className="block text-sm font-medium text-slate-600 mb-1">Exam Type</label>
//               <select {...register('exam_type')} className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white">
...
import { HiOutlinePlusCircle, HiOutlineTrash, HiOutlinePencil } from 'react-icons/hi';
import { getExams, createExam, updateExam, deleteExam } from '../../api/examApi';
import { getCourses } from '../../api/courseApi'; // FK Data
import { getClassrooms } from '../../api/classroomApi'; // FK Data
import { examSchema } from '../../validation/examSchema';
import Modal from '../../components/ui/Modal';
import Spinner from '../../components/ui/Spinner';
