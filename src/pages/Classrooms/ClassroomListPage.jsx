...
//     try { setLoading(true); const res = await getClassrooms(); setClassrooms(res.data); } 
//     catch { toast.error('Failed to fetch'); } finally { setLoading(false); }
//   };

//   useEffect(() => { fetchClassrooms(); }, []);

//   const openCreateModal = () => { setEditingClass(null); reset({ classroom_id: '', room_number: '', building_name: '', capacity: '', type: '' }); setIsModalOpen(true); };
...
//           <div><label className="block text-sm font-medium text-slate-600 mb-1">Classroom ID</label><input {...register('classroom_id')} disabled={editingClass} className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 disabled:bg-slate-100" placeholder="ROOM003" />{errors.classroom_id && <p className="text-red-500 text-xs mt-1">{errors.classroom_id.message}</p>}</div>
...
//             <select {...register('type')} className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white">
